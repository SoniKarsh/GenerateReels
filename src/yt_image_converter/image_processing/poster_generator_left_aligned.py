...
def draw_centered_left_aligned_text(image, day_number):
    ...
    main_text_bbox = draw.multiline_textbbox((0, 0), main_text, font=font_large, spacing=20, align="left")
    day_text_bbox = draw.textbbox((0, 0), day_text, font=font_small)

    block_width = max(main_text_bbox[2], day_text_bbox[2])
    total_height = (main_text_bbox[3] - main_text_bbox[1]) + (day_text_bbox[3] - day_text_bbox[1]) + 30

    # Top-left coordinates for center-aligned block
    block_x = (WIDTH - block_width) // 2
    main_text_y = (HEIGHT - total_height) // 2
    day_text_y = main_text_y + (main_text_bbox[3] - main_text_bbox[1]) + 30

    # Draw texts left-aligned within the centered block
    draw.multiline_text((block_x, main_text_y), main_text, font=font_large, fill="white", spacing=20, align="left")
